...
%% Create System Objects
    function obj = setupSystemObjects()
        ...
        obj.reader = vision.VideoFileReader('./PETS2014-0101-2.mp4');
        ...
        obj.blobAnalyser = vision.BlobAnalysis('BoundingBoxOutputPort', true, ...
            'CentroidOutputPort', true, 'MinimumBlobArea', 100);
    end
...
%% Check For Future Accident
    function checkForFutureAccident ()
        ...
        % for i=1:10
            % for each track
                % calc delta between last two frames
                % prediction = delta * i + last frame
                % store that prediction
            % for each track - t1
                % for each track - t2
                    % if t1 and t2 overlapped - rectint(t1, t2)
                        % alert
        for i=1:10
